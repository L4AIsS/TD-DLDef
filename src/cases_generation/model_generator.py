    ...
    def generate(self, json_path: str, exp_dir: str, model_id: int, initial_weight_dir: Optional[str] = None):
        ...
        if initial_weight_dir is None:
            ...
            for bk in self.__backends:
                p = CmdProcess(f"/home/liyuanfei/miniconda3/envs/{bk}/bin/python -m src.cases_generation.generate_one"
                               f" --backend {bk}"
        ...
        cmd_processes = {
            bk: CmdProcess(f"/home/liyuanfei/miniconda3/envs/{bk}/bin/python -m src.cases_generation.generate_other"
                           f" --backend {bk}"
                           f" --json_path {str(json_path)}"
                           f" --output_dir {str(model_dir)}")
