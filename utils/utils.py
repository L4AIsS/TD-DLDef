...
def switch_backend(bk):
    ...
    if bk == 'tensorflow':
        os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0'  # 禁用oneDNN的优化。
        # os.environ["CUDA_VISIBLE_DEVICES"] = "-1" # 只使用 CPU
...
seq_layer_types = [
    ...
    # 'locally_connected1D', 3.0 unsupported
    # 'locally_connected2D', 3.0 unsupported
] # 37
...
# for seq
layer_types = seq_layer_types + RNN_layer_types + activation_layer_types
...
losses = [
    'mean_squared_error',
    'mean_absolute_error',
    # 'mean_absolute_percentage_error', # pytorch 不支持
    # 'mean_squared_logarithmic_error', # pytorch 不支持
    # 'squared_hinge', # pytorch 不支持
    # 'hinge',  # 这个loss专用于二分类
    # 'categorical_hinge', # pytorch 不支持
    # 'logcosh', 分析不出
    'huber_loss',
    ...
    'poisson',
    # 'cosine_proximity', # tf 和 pytorch 不支持
]
